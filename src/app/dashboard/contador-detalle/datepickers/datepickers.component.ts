...
@Component({
  selector: 'app-contador-datepickers',
  templateUrl: './datepickers.component.html',
  styleUrls: ['./datepickers.component.css']
})
export class DatepickersComponent2 implements OnInit {
 ...
 /**/
  ...
  ngOnInit(): void {
    ...
    this.contadorService.get_fecha('lknlkn'); 
  }
/***---------------------------------------------------------------------------------------------------
  *      FECHA ACTUAL
  ---------------------------------------------------------------------------------------------------*/
...
consulta(){
  ...
  if(dtp1.touched == false)
   {
      if(val_dtp2 > this.fecha_actual())
      {
        ...
        this.contadorService.get_fecha(val_dtp2);
      }
   }
    ...
    if(val_dtp1 > this.fecha_actual())
      {
        ...
        this.contadorService.get_fecha(val_dtp1);   
      }
  }
  ...
  if(dtp1.touched != false && dtp2.touched != false){
    ...
    //Si dtp1 y dtp2 son menores a la fecha actual 
    if(val_dtp1 <= this.fecha_actual() && val_dtp2 <= this.fecha_actual()){
      //Si dtp1 es menor igual a dtp2
      if(val_dtp1 < val_dtp2){
        let fechas = val_dtp1+"*"+val_dtp2;
        this.contadorService.get_fecha(fechas);    
      }
      //Si dtp2 es menor igual a dtp1
      if(val_dtp2 < val_dtp1){
        this.msg_error_fecha_desde_mayor()
      }
      //Si las fechas son igualess
      if(val_dtp1 == val_dtp2){
        this.contadorService.get_fecha(val_dtp1);
      }
